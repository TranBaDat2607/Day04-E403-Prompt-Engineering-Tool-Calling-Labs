...
def _normalize(text: str) -> str:
    """Lowercase, strip Vietnamese diacritics, and collapse to ascii words for matching."""
...
class OrderDataStore:
    """Local catalog + order persistence layer for the order agent.

    All randomness is deterministically seeded so the grader can compare the
    saved JSON, detail tokens, discounts, and order IDs against fixtures.
    """
    ...
    def build_detail_token(product_ids: list[str]) -> str:
        """Stable token over a (sorted) product-id set; later tools must echo it back."""
    ...
    def save_order(
        ...
        *,
        ...
        payload = {
            ...
            "notes": notes.strip(),
            # Always POSIX-style ("artifacts/orders/...") so the saved JSON matches the
            # fixtures on every OS — str(Path) yields backslashes on Windows and breaks
            # the exact-match comparison in the grader.
